...
const detailsSchema = z
  .object({
    ...
    minimumLevel4: z.enum(["yes", "no"]),
    haveWES: z.enum(["yes", "no"]),
    atLeast6Months: z
    .array(z.string())
    .min(1, { message: "At least one reason for immigration is required" }),
    sda: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, {
      message: "Start date must be in the format YYYY-MM-DD",
    }),
    eda: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, {
      message: "End date must be in the format YYYY-MM-DD",
    }),
    trained: z.enum(["yes", "no"]),
    offer: z.enum(["yes", "no"]),
    ...
    const onSubmit = async () => {
      ...
      const newBody = {
        ...
        "Additional Information": body.additionalInformation,
          "Do you have a minimum of level 4 in English or French in the CLB or NCLC": body.minimumLevel4,
          "Do you have WES or other Education Credential Assessment the equivalent of a Canadian high school diploma (or higher)": body.haveWES,
          "Do you  have at least 6 months of recent and relevant work experience? If yes, provide the supporting documents you are able to provide": body.atLeast6Months,
          "Start Date of above employment": body.sda,
          "End Date of above employment": body.eda,
          "Do you have related caregiver training credential?": body.trained,
          "Do you have an offer for a full-time home care job from an employer who meets LICO as per the family size and  additional income to pay your wages": body.offer,
      };
          ...
          <div className="flex flex-wrap gap-6 scroll">
            ...
            <div className="flex flex-col w-full">
                ...
                <select
                  multiple
                  size="5"
                  {...register("atLeast6Months")}
                  className="bg-gray-400 text-sm border-none w-[50%] px-4 pb-2 border rounded-md text-gray-700 focus:outline-none "
                >
                  <option value="Payslips">
                  Payslips
                  </option>
                  <option value="Bank deposits of the earnings">Bank deposits of the earnings</option>
                  <option value="Tax documents">Tax documents</option>
                  <option value="Employment letter">
                  Employment letter
                  </option>
    ...
    return (
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl text-dark font-bold mb-4">
          Consultation Form for caregivers
        </h2>
        <button
